...
1 <= |s| <= 100
Approach
The string can be traversed and the letter which is capital is pushed into the result string.

Algorithm:

Traverse the string from left to right.
For each letter, if the character of the string is capital, push it into the result string.
Check for all the elements.
Print the result string.
Time Complexity: O(n), as we are traversing the string once.

Space Complexity: O(n), for storing the result (worst case).
...
public class Autori {
    ...
    // Function to extract uppercase letters from the input string
    static String autori(String str){
        String result = ""; // To store the output (initials)
        ...
        // Return the final string of uppercase letters
        return result;
    }
    ...
    public static void main(String[] args) {
        ...
        String str = sc.next();               // Read one word/string as input

        String result = autori(str);          // Call the function to get initials

        System.out.print(result);             // Print the result (uppercase initials)
