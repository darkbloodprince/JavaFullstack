...
public class Autori {
    static String autori(String str) {
        ...
        // The first character is always uppercase, so add it
        sb.append(str.charAt(0));

        // Iterate through the string
        for (int i = 1; i < str.length(); i++) {
            // If current character is '-', the next character is uppercase initial
            if (str.charAt(i) == '-') {
                // Append the next character (initial of the next name)
